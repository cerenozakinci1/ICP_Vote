...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        let old_propsal_opt = p.borrow().get(&key);
        let old_propsal: Proposal;
        match old_propsal_opt {
            Some(value) => old_propsal = value,
            None => return Err(VoteError::NoSuchProposal),
        }
        if old_propsal.owner != ic_cdk::caller() {
            return Err(VoteError::AccessRejected);
        }
        let value = Proposal {
            description: proposal.description,
            approve: old_propsal.approve,
            reject: old_propsal.reject,
            pass: old_propsal.pass,
            is_active: proposal.is_active,
            voted: old_propsal.voted,
            owner: old_propsal.owner,
        };
...
fn end_proposal(key: u64) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        ...
        old_propsal.is_active = false;

        let res = p.borrow_mut().insert(key, old_propsal);
        match res {
            Some(_) => Ok(()),
            None => Err(VoteError::UpdateError),
        }
...
fn vote(key: u64, choice: Choice) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(|p| {
        let propsal_opt = p.borrow().get(&key);
